...
async def main():
    ...
    api = MarzbanAPI(base_url="http://example.com")

    # Получение токена администратора
    token = await api.get_token(username="admin", password="admin_password")
    ...
    new_admin = AdminCreate(username="new_admin", is_sudo=True, password="new_password", telegram_id=123456789, discord_webhook="https://discord.com/api/webhooks/...")
    ...
    modified_admin = await api.modify_admin(username="new_admin", admin=AdminModify(is_sudo=False, password="new_password2"), token=token.access_token)
    print("Modified Admin:", modified_admin)

    ## Удаление администратора
    await api.remove_admin(username="new_admin", token=token.access_token)
